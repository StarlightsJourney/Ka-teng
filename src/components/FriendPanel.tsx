import { friendName, type Friend, type FriendLink } from '../element'
...
export function FriendPanel({ friend, friends, links, onSelect, onClose }: FriendPanelProps) {
  ...
  return <aside className="details-panel friend-panel">
    <div className="friend-panel-head"><div className="friend-panel-avatar">{friend.avatar ? <img src={friend.avatar} alt="" referrerPolicy="no-referrer" /> : `${friend.firstName[0]}${friend.lastName[0]}`}</div><div><h1>{friendName(friend)}</h1><p>{ringLabel[friend.circle]}</p></div><button type="button" className="panel-close" onClick={onClose}>×</button></div>
