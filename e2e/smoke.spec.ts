...
test('Peng-yu layout fits a phone-sized viewport', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 })
  await page.goto('/')
  await page.locator('.brand').click()
  await expect(page.locator('.friends-sidebar')).toBeVisible({ timeout: 10000 })
  const sidebar = page.locator('.friends-sidebar')
  const box = await sidebar.boundingBox()
  expect(box?.height).toBeLessThanOrEqual(420)
})

test('Peng-yu add, edit, and remove a friend', async ({ page }) => {
  await page.goto('/')
  await page.locator('.brand').click()
  await expect(page.locator('.friends-sidebar')).toBeVisible({ timeout: 10000 })
  await page.locator('.top-bar .add-person-button').click()
  await expect(page.locator('.modal[role="dialog"]')).toBeVisible()
  await page.locator('.modal input[placeholder="First name"]').fill('Peng')
  await page.locator('.modal input[placeholder="Last name"]').fill('Yu')
  await page.locator('.modal .btn-primary').click()
  await expect(page.locator('.modal[role="dialog"]')).not.toBeVisible()
  await page.locator('.friends-sidebar button:has-text("Peng Yu")').click()
  await expect(page.locator('.friend-panel')).toBeVisible()
  await page.locator('.friend-panel .panel-edit').click()
  await expect(page.locator('.modal[role="dialog"]')).toBeVisible()
  await page.locator('.modal input[placeholder="First name"]').fill('Peng2')
  await page.locator('.modal .btn-primary').click()
  await expect(page.locator('.modal[role="dialog"]')).not.toBeVisible()
  await expect(page.locator('.friend-panel')).toContainText('Peng2 Yu')
  await page.locator('.friend-panel .panel-edit').click()
  await page.locator('.modal .btn-danger').click()
  await page.locator('.modal .btn-danger').click()
  await expect(page.locator('.friend-panel')).not.toBeVisible()
})
